...
export default function BoatsBuiltAdminPage() {
  ...
  async function loadBoats() {
    ...
    try {
      const response = await fetch('/api/admin/boats-built', {
        method: 'GET',
        cache: 'no-store',
      })
      ...
      if (text) {
        try {
          ...
          throw new Error(
            `Server returned invalid JSON. Response: ${text.substring(0, 300)}`
          )
        }
      }
      ...
      if (!response.ok) {
        throw new Error(
          result?.error ||
            `Failed to load boats. Server returned ${response.status}.`
        )
      }

      if (Array.isArray(result?.boats)) {
  ...
  async function saveEdit(id: number) {
    ...
    try {
       ...
       * IMPORTANT:
       *
       * We are NOT updating Supabase directly here.
       *
       * The request goes through our server API,
       * which uses SUPABASE_SECRET_KEY.
       */
      ...
      if (text) {
        try {
          ...
          throw new Error(
            `Server returned invalid JSON. Response: ${text.substring(0, 500)}`
          )
        }
      }
      ...
      /*
       * Replace the boat in the screen immediately.
       */
